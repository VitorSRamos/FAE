...
void dimuons()
{
  ...
  // ----------------------- Alteração -----------------------
  // Para rodar o espectro completo é necessário alterar este valor de 0 para -1
  a._sdig = -1;
  ...
  // ----------------------- Alteração -----------------------
  // Escolha do range do pico e mudança da extra string
  // Foi usado 9.1 a 9.8 para o ajuste gaussiano e 9.2 a 9.7 para o crystall ball
  a._mmin = 9.2; //9.1
  a._mmax = 9.7; //9.8
  extraString = "#Upsilon(1S)"; 	// NOTE: this is used in file name:
...
void dimuon::SaveHisto(TH1F* hist, Int_t log) {
  ...
  hist->GetXaxis()->SetTitle("massa invariante #mu^{+}#mu^{-} [GeV]");
  hist->GetYaxis()->SetTitle("Eventos / GeV");
...
void dimuon::SelectPeak()
{
  ...
  // ----------------------- Alteração -----------------------
  // variações na quantidade de bins informada na linha abaixo
  TH1F *hDimuonMass_peak = new TH1F("hDimuonMass_peak","hDimuonMass_peak",50,_mmin,_mmax); //qtd original de bins: 100
...
void dimuon::FitPeakRoofit()
{
  ...
  RooRealVar mass("mass","massa invariante #mu^{+}#mu^{-}",_mmin,_mmax,"GeV/c^{2}");
  ...
  // ------------------------ IMPLEMENTAÇÃO ORIGINAL DA PDF GAUSSIANA ------------------------
  ...
  // ------------------------ MINHA IMPLEMENTAÇÃO DA CRYSTALL BALL ------------------------
  // Criando parâmetros da PDF crystall ball  
  RooRealVar mean("mean","mean", 9.3, 8.0, 10);
  RooRealVar sigma("sigma","sigma", 0.3, 0.0001, 5.);
  ...
  legend->AddEntry(frame->findObject("dh"),"Dados","lpe");
  legend->AddEntry(frame->findObject("modelBkg"),"Ajuste BG","l");
  legend->AddEntry(frame->findObject("modelSig"),"Ajuste Sinal","l");
  legend->AddEntry(frame->findObject("model"),"Ajuste Global","l");
  legend->Draw();
  ...
  L.SetNDC();
  L.SetTextSize(0.04);
  L.DrawLatex(0.15,0.8,"Espectro Dimuon");
  L.SetTextSize(0.03);
  L.DrawLatex(0.15,0.75,"ressonancia: #Upsilon(1S)");
  L.DrawLatex(0.15,0.70,Form("massa: %5.3f #pm %5.3f GeV/c^{2}",
			     mean.getVal(), mean.getError()));
  L.DrawLatex(0.15,0.65,Form("largura: %5.3f #pm %5.3f MeV/c^{2}", 
			     sigma.getVal()*1000, sigma.getError()*1000));
  L.DrawLatex(0.15,0.60,Form("yield: %.0f #pm %.0f eventos", 
			     n_signal.getVal(), n_signal.getError()));


  roofit_canvas.SaveAs(("myplots/result_RooFit"+extraString+".png").c_str());
  roofit_canvas.SaveAs(("myplots/result_RooFit"+extraString+".pdf").c_str());
...
void dimuon::FitPeak()
{
  ...
  // make it pretty
  hpeak->GetXaxis()->SetTitle("massa invariante #mu^{+}#mu^{-} [GeV/c^{2}]");
  hpeak->GetYaxis()->SetTitle(Form("Eventos / %3.1f MeV/c^{2}",hpeak->GetBinWidth(1)*1000));
  hpeak->SetStats(0);
  hpeak->SetTitle("");
  ...
  legend->AddEntry(hpeak,"Dados","lpe");
  legend->AddEntry(backFcn,"Ajuste BG","l");
  legend->AddEntry(signalFcn,"Ajuste Sinal","l");
  legend->AddEntry(f,"Ajuste Global","l");
  legend->Draw("same");
  ...
  c0->SaveAs(("myplots/result_ROOT"+extraString+".png").c_str());
  c0->SaveAs(("myplots/result_ROOT"+extraString+".pdf").c_str());
